...
function getTCornerPositions(rotation: number): { x: number; y: number }[] {
  switch (rotation) {
    ...
    default: {
      // Exhaustive check: this should never be reached with valid rotation values
      const _exhaustiveCheck: never = rotation as never;
...
describe('TSpin', () => {
  ...
  describe('detectTSpin', () => {
    describe('non-T pieces should not trigger T-Spin', () => {
      it('returns type=none for I-piece and preserves wasRotation flag', () => {
      ...
      it('returns type=none for O-piece and preserves wasRotation flag', () => {
    ...
    describe('rotation requirement', () => {
      it('returns type=none when wasRotation=false, even with all corners filled', () => {
    ...
    describe('3-corner rule: requires at least 3 filled corners for T-Spin', () => {
      it('returns type=none with 0 corners filled', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
        // No corners filled
        const result = detectTSpin(tetromino, board, true, 0);
        expect(result.type).toBe('none');
      ...
      it('returns type=none with 1 corner filled', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
        // Fill only 1 corner
      ...
      it('returns type=none with 2 corners filled', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
        // Fill only 2 corners
      ...
      it('detects T-Spin with 3 corners filled (both front corners)', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
      ...
      it('detects T-Spin with all 4 corners filled', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
        // Fill all 4 corners
        fillTCorners(board, tetromino, [true, true, true, true]);
    ...
    describe('T-Spin Mini vs Full detection', () => {
      it('detects Mini when only 1 front corner is filled (3 total corners)', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
      ...
      it('detects Mini when kickIndex=4 (5th wall kick test), even with both front corners', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
      ...
      it('detects Mini when kickIndex>4 (future wall kick table extension)', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
        // Fill both front corners (would normally be full T-Spin)
        fillTCorners(board, tetromino, [true, true, true, false]);

        // kickIndex 5 should also be Mini (>= 4 condition)
        const result = detectTSpin(tetromino, board, true, 5);
        expect(result.type).toBe('mini');
      });

      it('detects Full when both front corners filled and kickIndex<4', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
    ...
    describe('T-Spin detection for all 4 rotation states', () => {
      it('detects Full T-Spin in rotation state 1 (R - T points right)', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
      ...
      it('detects Full T-Spin in rotation state 2 (180 - T points down)', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
      ...
      it('detects Full T-Spin in rotation state 3 (L - T points left)', () => {
        const tetromino = new Tetromino('T', { x: 4, y: 10 });
    ...
    describe('out-of-bounds corners count as filled (wall/edge detection)', () => {
      it('detects Full T-Spin at left wall (x=-1 corners are out of bounds)', () => {
      ...
      it('detects Full T-Spin at bottom edge (y=totalHeight corners are out of bounds)', () => {
      ...
      it('detects Full T-Spin at right wall (x=10 corners are out of bounds)', () => {
      ...
      it('detects Full T-Spin at top edge (y=-1 corners are out of bounds)', () => {
        // Place T-piece near top in rotation state 0 (T points up)
        // y: -1 so front corners at y=-1 are out of bounds
        const tetromino = new Tetromino('T', { x: 4, y: -1 });
        tetromino.setRotation(0);

        // Center is at (5, 0)
        // In rotation state 0 (T points up):
        // - Front corners at y=-1 (out of bounds) at x=4 and x=6
        // - Back corners at y=1 at x=4 and x=6
        // Fill 1 back corner to get 3 total (2 front out of bounds + 1 back filled)
        board.setCell(4, 1, true, '#ff0000');

        const result = detectTSpin(tetromino, board, true, 0);
        // Top edge provides 2 "filled" front corners (out of bounds)
        // Plus the one we set = 3 corners
        // Both front corners are out of bounds and count as filled, so it's full T-Spin
        expect(result.type).toBe('full');
      });
  ...
  describe('getTSpinDescription', () => {
    describe('no T-Spin', () => {
      ...
      it('should return "N Lines" for 5+ lines (default branch)', () => {
        expect(getTSpinDescription('none', 5)).toBe('5 Lines');
        expect(getTSpinDescription('none', 10)).toBe('10 Lines');
      });
    });
    ...
    describe('T-Spin Mini', () => {
      ...
      it('should return T-Spin Mini Triple for 3 lines', () => {
        expect(getTSpinDescription('mini', 3)).toBe('T-Spin Mini Triple');
      });

      it('should return "T-Spin Mini N Lines" for 4+ lines (default branch)', () => {
        expect(getTSpinDescription('mini', 4)).toBe('T-Spin Mini 4 Lines');
        expect(getTSpinDescription('mini', 5)).toBe('T-Spin Mini 5 Lines');
      });
    });
    ...
    describe('full T-Spin', () => {
      ...
      it('should return "T-Spin N Lines" for 4+ lines (default branch)', () => {
        expect(getTSpinDescription('full', 4)).toBe('T-Spin 4 Lines');
        expect(getTSpinDescription('full', 5)).toBe('T-Spin 5 Lines');
      });
    });
  });
});
